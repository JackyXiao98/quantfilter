...
from transformers.file_utils import is_datasets_available , is_torch_xla_available
...
from E2E_utils import decoding_utils
...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        ...
        if hasattr(self.optimizer, 'prestep'):
            loss= self.optimizer.prestep(closure)
        else:
            loss = closure()
