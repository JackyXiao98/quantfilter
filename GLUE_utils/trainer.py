...
class Trainer(transformers.Trainer):
    ...
    def train(self, model_path=None, dev_objective=None, dev_objective_key=None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                tr_loss += self.training_step(model, inputs, optimizer)#;print('>>>>>>>>>>>>>',losses)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], optimizer) -> dict:
        model.train()
        ...
        if hasattr(optimizer, 'prestep'):
            loss= optimizer.prestep(closure)
        else:
            loss = closure()
